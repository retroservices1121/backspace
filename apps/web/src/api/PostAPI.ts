...
import { 
  ...
  getDocs, limit, orderBy, query, setDoc, 
  startAfter, Timestamp, where } 
...
import { 
  Collections, CommentDocument, LikeDocument, 
  MediaType, MemberDocument, MessageType, 
  PermissionsType, PostDocument, WithId } 
...
const  placeholderProfile  = '../../public/graphics/placeholders/1.png';
...
async function getPostDocumentsByQuery(
  matchList: string[] = [], sorting: Sort = Sort.Discover, 
  count: number = 10, lastPostId: string = '') {
    ...
    case Sort.Discover:
      if (lastPostId?.length > 0) {
        ...
        posts = await postsCollection.queryWithId(
          where('permission_required', '==', PermissionsType.Anyone),
...
export async function getPosts(
  matchList: string[] = [], sorting: Sort = Sort.Discover, 
  memberships: WithId<MemberDocument>[] = [], count?: number, lastPostId?: string ) {
  ...
  return processPosts(documents, memberships);
}

export async function getMorePosts(
  lastPostId: string, matchList: string[] = [], sorting: Sort = Sort.Discover, 
  memberships: WithId<MemberDocument>[] = [], count: number = 20) {
...
export type CreatePostType = 
  Pick<PostDocument, 'text' | 'title' | 'disable_comments' | 'permission_required' | 'community' | 'channel'>;
export async function createPost(post: CreatePostType, user: OldUser, mediaFile?: File) {
...
export const getCommentsWithAuthor = async (
  postId: string, community: string, channel: string) : Promise<CommentUnion[]> => {

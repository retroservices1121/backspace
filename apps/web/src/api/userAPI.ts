...
import { NetworkDocument } from 'types/documents';
import { Collections, FollowDocument, MemberDocument, OnlinePresence, PermissionsType, PrivateUserDocument, StatusDocument, UserDocument, WithId } from 'types/documents';
...
export async function getUserById(userId: string): Promise<OldUser | null> {
...
export async function getUserByUsername(username: string): Promise<OldUser> {
  const users = await UserTable.query(
    where<UserDocument>('username', '==', username),
    limit(1),
  );
  const matchingUser = users?.[0] as OldUser;
  if (matchingUser) {
    return {
      ...matchingUser,
...
export async function findUsers(userInput: string): Promise<OldUser[]> {
...
export const getAllFollowedUsers = async (uid: string, getFollowing: boolean = false) => {
  ...
  return filteredList as Array<OldUser>;
};
...
export async function getUser(userId: string) {
  ...
  } as Omit<OldUser, 'isFetched' | 'isLoggedIn' | 'isWelcomed'>;
}

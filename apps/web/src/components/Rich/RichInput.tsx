...
import useAsyncEffect from '@src/hooks/useAsyncHook';
import { useSearch } from '@src/hooks/useSearch';
import useUser from '@src/hooks/useUser';
import { selectCommunityMembers } from '@src/store/community/selectors';
...
const RichInput = ({
  ...
  const [ suggestions, setSuggestions ] = useState<string[]>(
    user?.following?.map((each) => each.account.username) || [],
  );
  ...
  if (!editorRef.current) editorRef.current = 
    withShortcuts(withLinks(withMentions(withReact(withHistory(createEditor())))));
  ...
  const handleSubmit = () => {
    if (!send) return null;
    const stringLength = Editor.string(editor, []).length;
    let submitted = false;
    if (stringLength === 0) {
      submitted = onSubmit([]);
    } else {
  ...
  //TODO have this searchText the db for more usernames
  useAsyncEffect([searchText], async () => {
    if (searchText) {
      const matches = await search.users(searchText);
      console.log(matches);
      setSuggestions(matches?.map((each) => each.username));
    } else {
      setSuggestions([]);
    }
  });
  ...
  const chars = suggestions.filter(c =>
    c.toLowerCase().startsWith(searchText.toLowerCase()),
  ).slice(0, 10);
      ...
      if (target) { // if in the middle of an @mention
        switch (event.key) {
          ...
          case 'Enter':
            ...
            insertMention(editor, chars[index] || searchText);
            setTarget(undefined);
            focusInput();
            break;
          case 'Escape':
  ...
  }, [chars.length, editor, index, searchText, target]);
          ...
          <Portal>
            <div
              ...
              style={{
                top: '-9999px',
                left: '-9999px',
                marginTop: `${-30 * (chars.length + 1)}px`,
                position: 'absolute',
